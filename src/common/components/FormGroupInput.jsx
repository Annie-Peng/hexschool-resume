...
const FormGroupInput = ({ groupDataSet, register, errors, formClass, edit, ...props}) => {
        ...
        <div className={`relative ${containerClass}`}>
          {group.map((formData, index)=>{
            ...
            if(typeof formData.group === "object") { // group表單
              return <FormGroupInput key={index} groupDataSet={formData} register={register} errors={errors} formClass={formClass.group[index]} edit={edit} {...props} />
            }
            ...
            return (
              <Fragment key={index}>
                  {RenderForm && <RenderForm formData={formData} register={register} error={ errors?.[id]?.[key]?.[item] || errors?.[id]?.[key] } formClass={formClass.group[index]} edit={edit} {...props}/>}
                  {edit && index === 0 && (
